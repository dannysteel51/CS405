// Module 2 Assign 1.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
// New function added for detecting SQL injection detection
bool injection_detection(const std::string& sql)
{
    int sqlStringPosition = 0;
    std::string where = "where";
    std::string localCopy(sql);                 // local copy of sql statement
    std::transform(localCopy.begin(), localCopy.end(), localCopy.begin(), ::tolower);       // make all lowercase
    std::size_t found_string = localCopy.find(where);

    if (found_string != std::string::npos)          // WHERE
    {
        std::string id = "id";                  // Search String for the Column 'ID'
        std::string name = "name";              // Search String for the Column 'NAME'
        std::string password = "password";      // Search String for the Column 'PASSWORD'
        std::string or_string = " or ";             // Search String for the Column 'OR'
        std::size_t whereFound = found_string;      // Make a copy of the where location

        // Check for ID after WHERE
        found_string = localCopy.find(id, whereFound);
        if (found_string != std::string::npos)
        {
            // Find OR
            found_string = localCopy.find(or_string, found_string);
            if (found_string != std::string::npos)
            {
                // Injection found
                return true;
        ...
        // Check for NAME after WHERE
        found_string = localCopy.find(name, whereFound);
        if (found_string != std::string::npos)
        {
            // Find OR
            found_string = localCopy.find(or_string, found_string);
        ...
        // Check for PASSWORD after WHERE
        found_string = localCopy.find(password, whereFound);
        if (found_string != std::string::npos)
        {
            // Find OR
            found_string = localCopy.find(or_string, found_string);
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    if (injection_detection(sql))
    {
        std::cout << std::endl;
        std::cout << "Data failed to be queried from USERS table. ERROR = SQL Injection Detected!!!" << std::endl;
        return false;
    }
